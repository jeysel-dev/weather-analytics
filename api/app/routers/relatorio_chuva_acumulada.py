...
- GET /api/v1/relatorio-chuva-acumulada/dados -> ranking de precipitação
  ...
  todas do estado, ou uma cidade só), ordenado do maior para o menor.
...
from app.routers.ref import cidade_filter, meso_filter
...
def get_dados(
    ...
    cidade: str | None = Query(None, description="Município (omitir ou 'Todas' = sem filtro)"),
) -> RelatorioChuvaAcumuladaResponse:
    anchor = max_date(_DAILY)
    if anchor is None:
        return RelatorioChuvaAcumuladaResponse()
    clause, meso_params = meso_filter(meso)
    cidade_clause, cidade_params = cidade_filter(cidade)
    ...
    rows = query(
        ...
        """,
        params={"max_date": anchor, "dias": dias, **meso_params, **cidade_params},
    )
    return RelatorioChuvaAcumuladaResponse(
